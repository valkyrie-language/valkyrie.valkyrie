...
export function package_compiler_createCompilerOptions(outputFormat, optimize, debug) {
return {"outputFormat": (outputFormat || "js"), "optimize": (optimize || false), "debug": (debug || false)};
}
...
return {"tokensCount": 0, "astNodesCount": 0, "compilationTime": 0, "outputSize": 0};
}
...
export function package_compiler_compileSourceWithOptions(source, outputFormat, optimize, debug) {
let compiler = {"source": source, "multiFile": false};
let options = package_compiler_createCompilerOptions(outputFormat, optimize, debug);
...
return {"namespaces": {}, "usings": {}, "currentNamespace": "", "currentFile": "", "mode": "repl"};
}
...
export function package_compiler_addSymbolToNamespace(manager, namespacePath, symbolName, symbolType, symbolData, filePath) {
...
namespaceData.symbols[symbolName] = {"type": symbolType, "data": symbolData, "filePath": filePath};
...
export function package_compiler_resolveSymbol(manager, symbolName, currentNamespace, currentFile) {
...
if ((symbol != null)) {
return {"found": true, "symbol": symbol, "namespace": usingNamespace};
}
}
i = (i + 1);
}
...
export function package_compiler_findSymbolNamespace(manager, symbolName, symbolType) {
...
export function package_compiler_getFullyQualifiedName(manager, symbolName, namespacePath) {
...
package_compiler_addSymbolToNamespace(manager, manager.currentNamespace, stmt.name, "function", stmt, manager.currentFile);
} else if ((stmt.type == "ClassDeclaration")) {
package_compiler_addSymbolToNamespace(manager, manager.currentNamespace, stmt.name, "class", stmt, manager.currentFile);
} else if ((stmt.type == "LetStatement")) {
package_compiler_addSymbolToNamespace(manager, manager.currentNamespace, stmt.name, "variable", stmt, manager.currentFile);
}
i = (i + 1);
}
let j = 0;
...
stmt.uniqueName = package_compiler_getFullyQualifiedName(manager, stmt.name, manager.currentNamespace);
} else if ((stmt.type == "ClassDeclaration")) {
stmt.uniqueName = package_compiler_getFullyQualifiedName(manager, stmt.name, manager.currentNamespace);
} else if ((stmt.type == "LetStatement")) {
stmt.uniqueName = package_compiler_getFullyQualifiedName(manager, stmt.name, manager.currentNamespace);
...
let validationResult = package_compiler_validateNamespaceRules(ast, mode);
...
if ((manager.namespaces[currentNamespace] == null)) {
manager.namespaces[currentNamespace] = {"symbols": {}, "files": []};
}
} else if ((stmt.type == "UsingStatement")) {
let usingPath = package_codegen_joinPath(stmt.path, "::");
let isGlobal = package_compiler_isMainNamespace(usingPath);
...
package_compiler_addSymbolToNamespace(manager, currentNamespace, stmt.name, "variable", stmt, fileName);
} else if ((stmt.type == "MicroDeclaration")) {
stmt.sourceNamespace = currentNamespace;
functionStatements.push(stmt);
package_compiler_addSymbolToNamespace(manager, currentNamespace, stmt.name, "function", stmt, fileName);
} else if ((stmt.type == "ClassDeclaration")) {
stmt.sourceNamespace = currentNamespace;
classStatements.push(stmt);
package_compiler_addSymbolToNamespace(manager, currentNamespace, stmt.name, "class", stmt, fileName);
} else {
...
funcStmt.uniqueName = package_compiler_getFullyQualifiedName(manager, funcStmt.name, (funcStmt["namespace"] || ""));
k = (k + 1);
}
let l = 0;
while ((l < classStatements.length)) {
let classStmt = classStatements[l];
classStmt.uniqueName = package_compiler_getFullyQualifiedName(manager, classStmt.name, (classStmt["namespace"] || ""));
l = (l + 1);
}
let m = 0;
while ((m < variableStatements.length)) {
let varStmt = variableStatements[m];
varStmt.uniqueName = package_compiler_getFullyQualifiedName(manager, varStmt.name, (varStmt["namespace"] || ""));
m = (m + 1);
}
...
variableStatements.push(stmt);
package_compiler_addSymbolToNamespace(manager, currentNamespace, stmt.name, "variable", stmt, manager.currentFile);
} else if ((stmt.type == "MicroDeclaration")) {
functionStatements.push(stmt);
package_compiler_addSymbolToNamespace(manager, currentNamespace, stmt.name, "function", stmt, manager.currentFile);
} else if ((stmt.type == "ClassDeclaration")) {
classStatements.push(stmt);
package_compiler_addSymbolToNamespace(manager, currentNamespace, stmt.name, "class", stmt, manager.currentFile);
} else {
executionStatements.push(stmt);
}
i = (i + 1);
...
funcStmt.uniqueName = package_compiler_getFullyQualifiedName(manager, funcStmt.name, currentNamespace);
j = (j + 1);
}
let k = 0;
while ((k < classStatements.length)) {
let classStmt = classStatements[k];
classStmt.uniqueName = package_compiler_getFullyQualifiedName(manager, classStmt.name, currentNamespace);
k = (k + 1);
}
let l = 0;
while ((l < variableStatements.length)) {
let varStmt = variableStatements[l];
varStmt.uniqueName = package_compiler_getFullyQualifiedName(manager, varStmt.name, currentNamespace);
l = (l + 1);
}
let integratedAst = {"type": "Program", "statements": []};
let m = 0;
...
let resolvedAst = package_compiler_resolveMultipleNamespacesAndGenerate(ast);
return resolvedAst;
}
...
while ((j < functionStatements.length)) {
let funcStmt = functionStatements[j];
let uniqueName = package_compiler_getFullyQualifiedName(manager, funcStmt.name, funcStmt.sourceNamespace);
...
while ((k < classStatements.length)) {
let classStmt = classStatements[k];
let uniqueName = package_compiler_getFullyQualifiedName(manager, classStmt.name, classStmt.sourceNamespace);
...
while ((l < variableStatements.length)) {
let varStmt = variableStatements[l];
let uniqueName = package_compiler_getFullyQualifiedName(manager, varStmt.name, varStmt.sourceNamespace);
...
export function package_compiler_generateSingleJSFromAnalysis(fileContents, mode) {
let manager = package_compiler_createNamespaceManager();
...
let usingPath = package_codegen_joinPath(stmt.path, "::");
let providerFile = package_compiler_findNamespaceProvider(usingPath, fileContents);
...
funcStmt.uniqueName = package_compiler_getFullyQualifiedName(manager, funcStmt.name, funcStmt.sourceNamespace);
m = (m + 1);
}
let n = 0;
while ((n < classStatements.length)) {
let classStmt = classStatements[n];
classStmt.uniqueName = package_compiler_getFullyQualifiedName(manager, classStmt.name, classStmt.sourceNamespace);
n = (n + 1);
}
let o = 0;
while ((o < variableStatements.length)) {
let varStmt = variableStatements[o];
varStmt.uniqueName = package_compiler_getFullyQualifiedName(manager, varStmt.name, varStmt.sourceNamespace);
o = (o + 1);
}
let integratedAst = {"type": "Program", "statements": []};
let p = 0;
...
if (((funcStmt.uniqueName != null) && (funcStmt.uniqueName != funcStmt.name))) {
let modifiedStmt = Object.assign({}, funcStmt);
modifiedStmt.name = funcStmt.uniqueName;
modifiedStmt.body = package_compiler_resolveIdentifiersInStatement(funcStmt.body, manager, functionStatements, variableStatements, classStatements);
integratedAst.statements.push(modifiedStmt);
} else {
let modifiedStmt = Object.assign({}, funcStmt);
modifiedStmt.body = package_compiler_resolveIdentifiersInStatement(funcStmt.body, manager, functionStatements, variableStatements, classStatements);
...
if (((classStmt.uniqueName != null) && (classStmt.uniqueName != classStmt.name))) {
...
let resolvedStmt = package_compiler_resolveIdentifiersInStatement(executionStatements[t], manager, functionStatements, variableStatements, classStatements);
...
return package_compiler_generateSingleJSFromAnalysis(fileContents, "standard");
}
export function package_compiler_createDependencyAnalyzer() {
return {"dependencies": {}, "reverseDependencies": {}};
}
export function package_compiler_findNamespaceProvider(namespacePath, fileContents) {
let fileNames = Object.keys(fileContents);
let i = 0;
while ((i < fileNames.length)) {
let fileName = fileNames[i];
...
let stmt = ast.statements[j];
if ((stmt.type == "NamespaceStatement")) {
let declaredNamespace = package_codegen_joinPath(stmt.path, "::");
...
if (!visited[fileName]) {
package_compiler_simpleDfsVisit(fileName, analyzer, visited, sorted);
}
j = (j + 1);
}
return {"error": null, "sorted": sorted};
}
export function package_compiler_resolveIdentifiersInExpression(expr, manager, functionStatements, variableStatements, classStatements) {
if ((expr == null)) {
return expr;
}
...
while ((i < functionStatements.length)) {
let funcStmt = functionStatements[i];
if (((funcStmt.name == functionName) && (funcStmt.uniqueName != null))) {
...
resolvedExpr.callee = package_compiler_resolveIdentifiersInExpression(expr.callee, manager, functionStatements, variableStatements, classStatements);
}
} else {
resolvedExpr.callee = package_compiler_resolveIdentifiersInExpression(expr.callee, manager, functionStatements, variableStatements, classStatements);
}
...
resolvedArgs.push(package_compiler_resolveIdentifiersInExpression(expr.arguments[k], manager, functionStatements, variableStatements, classStatements));
k = (k + 1);
}
...
let resolvedExpr = Object.assign({}, expr);
resolvedExpr.left = package_compiler_resolveIdentifiersInExpression(expr.left, manager, functionStatements, variableStatements, classStatements);
resolvedExpr.right = package_compiler_resolveIdentifiersInExpression(expr.right, manager, functionStatements, variableStatements, classStatements);
return resolvedExpr;
}
...
let resolvedExpr = Object.assign({}, expr);
resolvedExpr.object = package_compiler_resolveIdentifiersInExpression(expr.object, manager, functionStatements, variableStatements, classStatements);
return resolvedExpr;
}
...
if (((classStmt.name == expr.className) && (classStmt.uniqueName != null))) {
...
resolvedArgs.push(package_compiler_resolveIdentifiersInExpression(expr.arguments[n], manager, functionStatements, variableStatements, classStatements));
n = (n + 1);
}
}
resolvedExpr.arguments = resolvedArgs;
...
export function package_compiler_resolveIdentifiersInStatement(stmt, manager, functionStatements, variableStatements, classStatements) {
if ((stmt == null)) {
return stmt;
}
if ((stmt.type == "LetStatement")) {
let resolvedStmt = Object.assign({}, stmt);
resolvedStmt.value = package_compiler_resolveIdentifiersInExpression(stmt.value, manager, functionStatements, variableStatements, classStatements);
return resolvedStmt;
}
if ((stmt.type == "ExpressionStatement")) {
let resolvedStmt = Object.assign({}, stmt);
resolvedStmt.expression = package_compiler_resolveIdentifiersInExpression(stmt.expression, manager, functionStatements, variableStatements, classStatements);
return resolvedStmt;
}
...
let resolvedStmt = Object.assign({}, stmt);
resolvedStmt.condition = package_compiler_resolveIdentifiersInExpression(stmt.condition, manager, functionStatements, variableStatements, classStatements);
resolvedStmt.thenBranch = package_compiler_resolveIdentifiersInStatement(stmt.thenBranch, manager, functionStatements, variableStatements, classStatements);
if ((stmt.elseBranch != null)) {
resolvedStmt.elseBranch = package_compiler_resolveIdentifiersInStatement(stmt.elseBranch, manager, functionStatements, variableStatements, classStatements);
}
return resolvedStmt;
}
...
let resolvedStmt = Object.assign({}, stmt);
resolvedStmt.condition = package_compiler_resolveIdentifiersInExpression(stmt.condition, manager, functionStatements, variableStatements, classStatements);
resolvedStmt.body = package_compiler_resolveIdentifiersInStatement(stmt.body, manager, functionStatements, variableStatements, classStatements);
return resolvedStmt;
}
if ((stmt.type == "Block")) {
let resolvedStmt = Object.assign({}, stmt);
...
resolvedStatements.push(package_compiler_resolveIdentifiersInStatement(stmt.statements[i], manager, functionStatements, variableStatements, classStatements));
i = (i + 1);
}
...
export function package_compiler_simpleDfsVisit(filePath, analyzer, visited, sorted) {
if (visited[filePath]) {
return;
}
...
while (((lexer.current_char != "") && package_lexer_isWhitespace(lexer.current_char))) {
lexer.advance();
}
}
...
let result = "";
while (((lexer.current_char != "") && package_lexer_isAlphaNumeric(lexer.current_char))) {
...
let result = "";
while (((lexer.current_char != "") && package_lexer_isDigit(lexer.current_char))) {
result = (result + lexer.current_char);
lexer.advance();
}
return result;
...
if (((package_lexer_nextToken.type == "ASSIGN") || (package_lexer_nextToken.type == "SEMICOLON"))) {
let name = package_parser_expect(parser, "IDENTIFIER");
if ((name && (name.type == "ParseError"))) {
return name;
}
