...
if (!options) {
  options = new package_compiler_CompilerOptions("js", false, false, "warning", mode || "repl");
} else if (!options.mode) {
  options.mode = mode || "repl";
}
let compiler = new package_compiler_Compiler(options);
return compiler.compile_asts(file_contents);
}
...
class package_compiler_Compiler {
  ...
  compile_asts_with_diagnostics(file_contents) {
    ...
    let sort_result = package_compiler_topological_sort(this.dependency_analyzer, file_contents);
    ...
    while ((k < sorted_files.length)) {
      ...
      let validation_result = package_compiler_validate_namespace_rules(ast, this.options.mode);
      ...
      while ((l < ast.statements.length)) {
        ...
        if ((stmt.type == "NamespaceStatement")) {
          ...
          let using_path = package_compiler_join_path(stmt.path, "::");
          let is_global = package_compiler_is_main_namespace(using_path);
          package_compiler_add_using_import(this.namespace_manager, using_path, is_global);
          ...
          package_compiler_add_symbol_to_namespace(this.namespace_manager, current_namespace, stmt.name, "variable", stmt, file_name);
        } else if ((stmt.type == "MicroDeclaration")) {
          stmt.sourceNamespace = current_namespace;
          function_statements.push(stmt);
          package_compiler_add_symbol_to_namespace(this.namespace_manager, current_namespace, stmt.name, "function", stmt, file_name);
        } else if ((stmt.type == "ClassDeclaration")) {
          stmt.sourceNamespace = current_namespace;
          class_statements.push(stmt);
          package_compiler_add_symbol_to_namespace(this.namespace_manager, current_namespace, stmt.name, "class", stmt, file_name);
        } else {
    ...
    while ((m < function_statements.length)) {
      ...
      func_stmt.uniqueName = package_compiler_get_fully_qualified_name(this.namespace_manager, func_stmt.name, func_stmt.sourceNamespace);
      m = (m + 1);
    }
    let n = 0;
    while ((n < class_statements.length)) {
      let class_stmt = class_statements[n];
      class_stmt.uniqueName = package_compiler_get_fully_qualified_name(this.namespace_manager, class_stmt.name, class_stmt.sourceNamespace);
      n = (n + 1);
    }
    let o = 0;
    while ((o < variable_statements.length)) {
      let var_stmt = variable_statements[o];
      var_stmt.uniqueName = package_compiler_get_fully_qualified_name(this.namespace_manager, var_stmt.name, var_stmt.sourceNamespace);
      o = (o + 1);
    }
