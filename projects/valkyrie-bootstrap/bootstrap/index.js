...
this.setup_builtin_types();  }
  ...
  setup_builtin_types() {
this.builtin_types["bool"] = package__analyzer__Type.create_primitive("bool");
this.builtin_types["i32"] = package__analyzer__Type.create_primitive("i32");
this.builtin_types["i64"] = package__analyzer__Type.create_primitive("i64");
this.builtin_types["f32"] = package__analyzer__Type.create_primitive("f32");
this.builtin_types["f64"] = package__analyzer__Type.create_primitive("f64");
this.builtin_types["String"] = package__analyzer__Type.create_primitive("String");
this.builtin_types["Object"] = package__analyzer__Type.create_primitive("Object");
this.builtin_types["void"] = package__analyzer__Type.create_primitive("void");
this.builtin_types["unknown"] = package__analyzer__Type.create_primitive("unknown");
this.builtin_types["any"] = package__analyzer__Type.create_primitive("any");
}
...
symbol.data_type = this.type_factory.infer_from_literal(symbol.value);
}
if (!symbol.data_type) {
...
this.add_error((("Flags member '" + member.name) + "' must have integer value"));
}
i = (i + 1);
}
}
...
let member = symbol.members[i];
if ((member.value && !this.is_integer_value(member.value))) {
this.add_error((("Eidos member '" + member.name) + "' must have integer value"));
}
i = (i + 1);
}
}
...
this.add_error(((("Member '" + member_name) + "' not found in ") + left_type.name));
return;
}
symbol.data_type = this.type_factory.get_builtin_type("i32");
} else {
this.add_error("Static access (::) can only be used with flags or eidos types");
}
}
...
statement.superClassesUniqueNames.push(this.resolve_unique_name(super_class_name));
...
if (((!this.usings || !this.usings[file]) || (this.usings[file].length == 0))) {
return null;
}
let u = 0;
...
return null;
  ...
  resolve_namepath_in_statement(stmt, function_statements, variable_statements, class_statements, options, diagnostics) {
if ((stmt == null)) {
return stmt;
}
let resolved_stmt = Object.assign({}, stmt);
if (stmt.type === "LetStatement") {
resolved_stmt.value = this.resolve_namepath_in_expression(stmt.value, function_statements, variable_statements, class_statements, options, diagnostics);
return resolved_stmt;
} else if (stmt.type === "ReturnStatement") {
resolved_stmt.argument = this.resolve_namepath_in_expression(stmt.argument, function_statements, variable_statements, class_statements, options, diagnostics);
return resolved_stmt;
} else if (stmt.type === "ExpressionStatement") {
resolved_stmt.expression = this.resolve_namepath_in_expression(stmt.expression, function_statements, variable_statements, class_statements, options, diagnostics);
return resolved_stmt;
} else if (stmt.type === "IfStatement") {
resolved_stmt.condition = this.resolve_namepath_in_expression(stmt.condition, function_statements, variable_statements, class_statements, options, diagnostics);
resolved_stmt.consequent = this.resolve_namepath_in_statement(stmt.consequent, function_statements, variable_statements, class_statements, options, diagnostics);
if ((stmt.alternate != null)) {
resolved_stmt.alternate = this.resolve_namepath_in_statement(stmt.alternate, function_statements, variable_statements, class_statements, options, diagnostics);
}
return resolved_stmt;
} else if (stmt.type === "WhileStatement") {
resolved_stmt.condition = this.resolve_namepath_in_expression(stmt.condition, function_statements, variable_statements, class_statements, options, diagnostics);
resolved_stmt.body = this.resolve_namepath_in_statement(stmt.body, function_statements, variable_statements, class_statements, options, diagnostics);
return resolved_stmt;
...
resolved_body.push(this.resolve_namepath_in_statement(stmt.statements[i], function_statements, variable_statements, class_statements, options, diagnostics));
i = (i + 1);
}
...
resolved_member.value = this.resolve_namepath_in_expression(member.value, function_statements, variable_statements, class_statements, options, diagnostics);
...
let resolved_member = Object.assign({}, member);
if ((member.body != null)) {
resolved_member.body = this.resolve_namepath_in_statement(member.body, function_statements, variable_statements, class_statements, options, diagnostics);
}
...
if (((this.usings && this.usings[this.current_file]) && (this.usings[this.current_file].length > 0))) {
let u = 0;
...
resolved_expr.name_path = using_import.path.map(function(part) {
return {"type": "Identifier", "name": part};
});
resolved_expr.unique_name = package__generation__join_name_path(using_import.path, "_");
...
return expr;
  ...
  resolve_namepath_in_expression(expr, function_statements, variable_statements, class_statements, options, diagnostics) {
if ((expr == null)) {
return expr;
}
...
let resolved_expr = Object.assign({}, expr);
resolved_expr.left = this.resolve_namepath_in_expression(expr.left, function_statements, variable_statements, class_statements, options, diagnostics);
resolved_expr.right = this.resolve_namepath_in_expression(expr.right, function_statements, variable_statements, class_statements, options, diagnostics);
return resolved_expr;
}
...
let resolved_expr = Object.assign({}, expr);
resolved_expr.expression = this.resolve_namepath_in_expression(expr.expression, function_statements, variable_statements, class_statements, options, diagnostics);
return resolved_expr;
}
if ((expr.type == "NewExpression")) {
let resolved_expr = Object.assign({}, expr);
let resolved_className = this.resolve_namepath_using_imports(expr.className);
...
resolved_args.push(this.resolve_namepath_in_expression(expr.arguments[k], function_statements, variable_statements, class_statements, options, diagnostics));
k = (k + 1);
}
...
let resolved_expr = Object.assign({}, expr);
resolved_expr.operand = this.resolve_namepath_in_expression(expr.operand, function_statements, variable_statements, class_statements, options, diagnostics);
return resolved_expr;
}
if ((expr.type == "PropertyAccess")) {
let resolved_expr = Object.assign({}, expr);
resolved_expr.object = this.resolve_namepath_in_expression(expr.object, function_statements, variable_statements, class_statements, options, diagnostics);
return resolved_expr;
}
if (((expr.type == "StaticMethodCall") || (expr.type == "StaticPropertyAccess"))) {
let resolved_expr = Object.assign({}, expr);
resolved_expr.className = this.resolve_namepath_in_expression(expr.className, function_statements, variable_statements, class_statements, options, diagnostics);
...
let resolved_expr = Object.assign({}, expr);
resolved_expr.object = this.resolve_namepath_in_expression(expr.object, function_statements, variable_statements, class_statements, options, diagnostics);
resolved_expr.index = this.resolve_namepath_in_expression(expr.index, function_statements, variable_statements, class_statements, options, diagnostics);
return resolved_expr;
}
...
resolved_prop.value = this.resolve_namepath_in_expression(prop.value, function_statements, variable_statements, class_statements, options, diagnostics);
...
resolved_elements.push(this.resolve_namepath_in_expression(expr.elements[i], function_statements, variable_statements, class_statements, options, diagnostics));
i = (i + 1);
}
...
let resolved_expr = Object.assign({}, expr);
resolved_expr.value = this.resolve_namepath_in_expression(expr.value, function_statements, variable_statements, class_statements, options, diagnostics);
return resolved_expr;
}
...
resolved_arm.pattern = this.resolve_namepath_in_expression(arm.pattern, function_statements, variable_statements, class_statements, options, diagnostics);
resolved_arm.body = this.resolve_namepath_in_expression(arm.body, function_statements, variable_statements, class_statements, options, diagnostics);
...
return ((package__generation__join_name_path(namespace_path, "_") + "_") + name);
}
return ((package__generation__join_name_path(source_namespace, "_") + "_") + name);
}
...
this.namespaces[namespace_key][name] = {"type": type, "node": node, "file": file_name};
}
...
this.usings[this.current_file].push({"path": using_path, "is_global": is_global});
}

  get_symbol_from_namespace(namespace, name) {
let namespace_key = package__generation__join_name_path(namespace, "_");
if (((this.namespaces[namespace_key] != null) && (this.namespaces[namespace_key][name] != null))) {
...
return {"success": false, "error": "Standard mode requires at least one namespace declaration"};
}
if (((mode == "standard") && !has_main_namespace)) {
return {"success": false, "error": "Standard mode requires exactly one main namespace (ending with !)"};
}
return {"success": true, "error": null};
}
export function package__compiler__find_namespace_provider(namespace_path, file_contents) {
let file_names = Object.keys(file_contents);
let i = 0;
while ((i < file_names.length)) {
...
let path_key = package__compiler__make_namespace_key(current_namespace);
let group = namespace_groups[path_key];
if (!group) {
group = new package__compiler__NamespaceGroup(current_namespace);
...
let code_generator = new package__generation__JsCodeGeneration("  ", compiler.options);
let generated_code = code_generator.generate(combined_ast);
console.log("=== 代码生成完成 ===");
return {"success": !compiler.diagnostics.has_errors(), "diagnostics": compiler.diagnostics.get_all_diagnostics(), "code": generated_code};
}
...
this.diagnostics.push({"level": package__compiler__DiagnosticLevel.ERROR, "message": message, "line": line, "column": column});
}

  add_warning(message, line, column) {
this.diagnostics.push({"level": package__compiler__DiagnosticLevel.WARNING, "message": message, "line": line, "column": column});
}

  has_errors() {
let i = 0;
while ((i < this.diagnostics.length)) {
if ((this.diagnostics[i].level == package__compiler__DiagnosticLevel.ERROR)) {
return true;
}
i = (i + 1);
}
...
let i = 0;
while ((i < this.diagnostics.length)) {
if ((this.diagnostics[i].level == package__compiler__DiagnosticLevel.WARNING)) {
return true;
}
i = (i + 1);
}
...
this.output_format = (output_format || package__compiler__OutputFormat.JS);
...
return {"type": "CompilerError", "message": message, "line": line, "column": column};
}
export function package__compiler__create_warning(message, line, column) {
return {"type": "CompilerWarning", "message": message, "line": line, "column": column};
}
...
count = (count + package__compiler__count_ast_nodes(node.statements[i]));
i = (i + 1);
}
}
if ((node.body != null)) {
...
return package__compiler__compile_asts_with_options(file_contents, mode, null);
}
...
export function package__compiler__generate_single_js_with_options(file_contents, options) {
return package__compiler__compile_asts_with_options(file_contents, "repl", options);
}
export function package__compiler__generate_single_js_standard_with_options(file_contents, options) {
return package__compiler__compile_asts_with_options(file_contents, "standard", options);
}
export function package__compiler__compile_text(source_text) {
let file_contents = {"main.valkyrie": source_text};
return package__compiler__compile_asts(file_contents, "repl");
}
export function package__compiler__compile_text_with_options(source_text, options) {
let file_contents = {"main.valkyrie": source_text};
return package__compiler__compile_asts_with_options(file_contents, "repl", options);
}
export function package__compiler__compile_asts_with_options(file_contents, mode, options) {
if ((options == null)) {
options = new package__compiler__CompilerOptions("js", false, false, (mode || "repl"));
...
class package__analyzer__rtta__AdtVariantDescriptor {
  constructor() {
...
class package__analyzer__rtta__MethodDescriptor {
  constructor() {
    self.name = undefined;
...
class package__analyzer__rtta__TraitDescriptor {
  constructor() {
    self.name = undefined;
...
class package__analyzer__rtta__TypeAliasDescriptor {
  constructor() {
    self.name = undefined;
...
class package__analyzer__rtta__ValkyrieTypeDescriptor {
  constructor() {
    self.name = undefined;
    self.size = undefined;
...
return position;
  ...
  has_valid_position() {
return ((((this.offset >= 0) && (this.length > 0)) && (this.line > 0)) && (this.column > 0));
}
...
let node_id = this.get_node_id(node);
if ((this.node_properties[node_id] && this.node_properties[node_id][key])) {
...
export function package__parser__parseExpressionWithPrecedence(parser, minPrecedence, inline) {
let left = package__parser__parseUnaryExpression(parser, inline);
if ((left && (left.type == "ParseError"))) {
return left;
}
while (true) {
let precedence = package__parser__getOperatorPrecedence(parser.current_token.type);
...
right = package__parser__parseExpressionWithPrecedence(parser, nextMinPrecedence, inline);
...
let arg = package__parser__parseExpressionWithPrecedence(parser, 0, inline);
...
parser.advance();
arg = package__parser__parseExpressionWithPrecedence(parser, 0, inline);
if ((arg.type == "ParseError")) {
return arg;
}
args.push(arg);
...
parser.advance();
let index = package__parser__parseExpressionWithPrecedence(parser, 0, inline);
...
if (((parser.current_token.type == "SYMBOL_XID") || (parser.current_token.type == "SYMBOL_RAW"))) {
...
if ((parser.current_token.type == "LPAREN")) {
parser.advance();
let expr = package__parser__parseExpressionWithPrecedence(parser, 0, inline);
if ((expr && (expr.type == "ParseError"))) {
return expr;
}
let rparen = parser.expect("RPAREN");
...
let element = package__parser__parseExpressionWithPrecedence(parser, 0, inline);
...
while ((parser.current_token.type == "COMMA")) {
parser.advance();
element = package__parser__parseExpressionWithPrecedence(parser, 0, inline);
if ((element && (element.type == "ParseError"))) {
return element;
}
node.elements.push(element);
...
let value = package__parser__parseExpressionWithPrecedence(parser, 0, inline);
...
let error = {};
error.type = "ParseError";
error.message = ("Expected when, case, type, or else in match branch but got " + branchType);
error.line = parser.current_token.line;
error.column = parser.current_token.column;
return error;
}
...
let statements = [];
while (((((((parser.current_token.type != "WHEN") && (parser.current_token.type != "CASE")) && (parser.current_token.type != "TYPE")) && (parser.current_token.type != "ELSE")) && (parser.current_token.type != "RBRACE")) && (parser.current_token.type != "EOF"))) {
...
export function package__parser__parseTypeExpressionWithPrecedence(parser, minPrecedence) {
let left = package__parser__parseUnaryTypeExpression(parser);
if ((left && (left.type == "ParseError"))) {
return left;
}
while (true) {
let precedence = package__parser__getTypeOperatorPrecedence(parser.current_token.type);
if ((precedence < minPrecedence)) {
break;
}
let op = parser.current_token.value;
...
let right = package__parser__parseTypeExpressionWithPrecedence(parser, nextMinPrecedence);
if ((right && (right.type == "ParseError"))) {
return right;
}
if ((tokenType == "ARROW")) {
...
let error = {};
error.type = "ParseError";
error.message = ((("Expected " + tokenType) + " but got ") + this.current_token.type);
...
return token;
  ...
  make_node(node_type) {
return new package__parser__Node(node_type, this.current_token.offset, this.current_token.length, this.current_token.line, this.current_token.column);
}
...
return package__parser__parse_program(this);
  ...
  parse_identifier() {
if (((this.current_token.type == "SYMBOL_XID") || (this.current_token.type == "SYMBOL_RAW"))) {
let token = this.current_token;
this.advance();
return token;
}
let error = {};
error.type = "ParseError";
error.message = ("Expected identifier but got " + this.current_token.type);
error.line = this.current_token.line;
error.column = this.current_token.column;
return error;
}
...
if ((context == "Branches")) {
if (((((this.current_token["type"] == "TYPE") || (this.current_token["type"] == "CASE")) || (this.current_token["type"] == "WHEN")) || (this.current_token["type"] == "ELSE"))) {
let token = this.current_token;
this.advance();
return token;
}
...
let error = {};
error["type"] = "ParseError";
error.message = ((("Expected identifier in context " + context) + " but got ") + this.current_token["type"]);
error.line = this.current_token.line;
error.column = this.current_token.column;
return error;
}
...
export function package__parser__parse_keyword(parser, expected_keyword, node_type) {
parser.advance();
let name = parser.parse_identifier();
if ((name && (name.type == "ParseError"))) {
return name;
...
parser.advance();
node.superClasses = [];
while (((parser.current_token.type != "RPAREN") && (parser.current_token.type != "EOF"))) {
let superName = parser.parse_identifier();
if ((superName && (superName.type == "ParseError"))) {
return superName;
}
node.superClasses.push(superName.value);
if ((parser.current_token.type == "COMMA")) {
parser.advance();
} else {
break;
...
let node = package__parser__parse_keyword(parser, "class", "ClassDeclaration");
...
let node = package__parser__parse_keyword(parser, "singleton", "SingletonDeclaration");
if ((node && (node.type == "ParseError"))) {
return node;
}
node = package__parser__parse_inheritor(parser, node);
...
let node = package__parser__parse_keyword(parser, "trait", "TraitDeclaration");
if ((node && (node.type == "ParseError"))) {
return node;
}
node = package__parser__parse_inheritor(parser, node);
...
if ((parser.current_token.type == "COLON")) {
parser.advance();
typeAnnotation = package__parser__parseTypeExpression(parser);
...
this.source_map_builder = new package__generation__SourceMapBuilder();
this.js_mapping = new package__generation__JsSourceMapping(this.source_map_builder);
} else {
...
if ((newline_count > 0)) {
this.js_mapping.current_line = (this.js_mapping.current_line + newline_count);
this.js_mapping.current_column = 0;
} else {
this.js_mapping.current_column = (this.js_mapping.current_column + text.length);
}
}
}
...
this.js_mapping.current_column = (this.js_mapping.current_column + full_line.length);
}
}
}
...
return new package__generation__SourceSpan(file_name, node.line, node.column, end_line, end_column);
} else {
return false;
}
}
...
this.js_mapping.update_position(this.js_mapping.current_line, (this.js_mapping.current_column + this.buffer.length));
this.js_mapping.source_map_builder.add_span_mapping(this.js_mapping.current_line, this.js_mapping.current_column, source_span, source_index);
this.buffer = (this.buffer + identifier);
this.js_mapping.current_column = (this.js_mapping.current_column + identifier.length);
} else {
this.buffer = (this.buffer + identifier);
}
}
...
result = ((((result + "  ") + member.name) + ": ") + this.generate_expression(member.value));
i = (i + 1);
}
result = (result + "\n};\n");
...
result = ((((result + "\"") + prop.key) + "\": ") + this.generate_expression(prop.value));
i = (i + 1);
}
result = (result + "}");
return result;
...
return (("// using " + package__generation__join_name_path(node.path, "__")) + ";");
}
...
let superClasses = node.superClassesUniqueNames;
let members = node.members;
...
let initValue = this.generate_expression(member.initializer);
...
result = (result + this.apply_mixins(className, superClasses));
}
return result;
}

  apply_mixins(targetClass, mixins) {
let mixinCode = "";
let i = 0;
...
result = (result + this.generate_branch_statements(branch.statements));
result = (result + "}");
...
let pattern_value = this.generate_pattern_expression(branch.pattern);
...
this.buffer = (this.buffer + this.js_mapping.generate_with_mapping(text, span, source_index));
...
this.source_files[file_name] = this.source_map_builder.add_source(file_name, content);
}
...
if (this.source_map_builder) {
this.source_map_builder.add_mapping(this.current_line, this.current_column, source_index, this.current_line, this.current_column, false);
}
}

  generate_with_mapping(code, source_span, source_index) {
if ((source_span && source_span.is_valid())) {
this.source_map_builder.add_span_mapping(this.current_line, this.current_column, source_span, source_index);
}
let i = 0;
let newline_count = 0;
while ((i < code.length)) {
...
this.mappings = mappings;
  ...
  to_json() {
return (((((("{\"version\":" + this.version) + ",\"sources\":[],\"sourcesContent\":[],\"names\":[],\"mappings\":\"") + this.mappings) + "\",\"file\":\"") + this.file) + "\"}");
}
...
this.source_map.set_file(file_name);
  ...
  add_mapping(generated_line, generated_column, source_index, original_line, original_column, name_index) {
let mapping = {"generated_line": generated_line, "generated_column": generated_column, "source_index": source_index, "original_line": original_line, "original_column": original_column, "name_index": name_index};
this.mappings.push(mapping);
}

  add_span_mapping(generated_line, generated_column, source_span, source_index) {
if ((source_span && source_span.is_valid())) {
this.add_mapping(generated_line, generated_column, source_index, source_span.start_line, source_span.start_column, false);
}
}
...
return false;
  ...
  to_string() {
return ((((((((this.file_name + ":") + this.start_line) + ":") + this.start_column) + "-") + this.end_line) + ":") + this.end_column);
}
...
let i = 0;
while ((i < node.children.length)) {
enhanced.children.push(this.enhance_node_with_symbol_info(node.children[i]));
i = (i + 1);
}
}
return enhanced;
...
let i = 0;
while ((i < node.arguments.length)) {
enhanced.arguments.push(this.enhance_node_with_symbol_info(node.arguments[i]));
i = (i + 1);
}
}
if (node.body) {
...
this.current_char = this.source.charAt(this.position);
  ...
  skip_whitespace() {
while (((this.current_char != "") && package__lexer__is_whitespace(this.current_char))) {
this.advance();
}
}
  ...
  read_number() {
let result = "";
while (((this.current_char != "") && package__lexer__is_digit(this.current_char))) {
...
return result;
  ...
  read_identifier() {
let result = "";
while (((this.current_char != "") && package__lexer__is_alpha_numeric(this.current_char))) {
result = (result + this.current_char);
this.advance();
}
return result;
...
return new package__lexer__Token(tokenType, value, line, column);
}
...
return new package__lexer__Token("SYMBOL_RAW", value, line, column);
}
...
this.advance();
return new package__lexer__Token("DOUBLE_COLON", "::", line, column);
}
...
if (((this.position < this.source.length) && (this.source.charAt(this.position) == ">"))) {
this.advance();
return new package__lexer__Token("ARROW", "->", line, column);
}
...
return new package__lexer__Token("ERROR", ("Unknown character: " + ch), line, column);
}
...
let constant_folding = new package__optimizer__transforms__ConstantFoldingTransform();
...
class package__optimizer__transforms__ConstantFoldingTransform {
  constructor() {
this.base_transform = new package__optimizer__Transform("ConstantFolding");  }
