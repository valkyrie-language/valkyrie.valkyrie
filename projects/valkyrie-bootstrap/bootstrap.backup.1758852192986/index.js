...
export function package_compiler_add_symbol_to_namespace(manager, namespace_path, symbol_name, symbol_type, symbol_data, file_path) {
...
namespace_data.symbols[symbol_name] = {"type": symbol_type, "data": symbol_data, "filePath": file_path};
...
export function package_compiler_add_using_import(manager, using_path, is_global) {
...
export function package_compiler_find_symbol_namespace(manager, symbol_name, symbol_type) {
...
export function package_compiler_get_fully_qualified_name(manager, symbol_name, namespace_path) {
if (((namespace_path == "") || (namespace_path == null))) {
return symbol_name;
}
let clean_namespace = package_compiler_get_main_namespace_name(namespace_path);
...
export function package_compiler_find_namespace_provider(namespace_path, file_contents) {
let file_names = Object.keys(file_contents);
let i = 0;
while ((i < file_names.length)) {
...
if ((symbol != null)) {
return {"found": true, "symbol": symbol, "namespace": current_namespace};
}
}
...
if ((symbol != null)) {
return {"found": true, "symbol": symbol, "namespace": using_namespace};
}
}
i = (i + 1);
}
...
export function package_compiler_resolve_identifiers_in_expression(expr, manager, function_statements, variable_statements, class_statements, options) {
if ((expr == null)) {
return expr;
}
...
let warning = package_compiler_create_warning((((((("Implicit member call detected for '" + function_name) + "'. Consider using 'Self::") + function_name) + "()' for static methods or 'self.") + function_name) + "()' for instance methods."), (expr.line || 0), (expr.column || 0));
...
if (!found_function) {
resolved_expr.callee = package_compiler_resolve_identifiers_in_expression(expr.callee, manager, function_statements, variable_statements, class_statements, options);
}
} else {
resolved_expr.callee = package_compiler_resolve_identifiers_in_expression(expr.callee, manager, function_statements, variable_statements, class_statements, options);
}
...
resolved_args.push(package_compiler_resolve_identifiers_in_expression(expr.arguments[k], manager, function_statements, variable_statements, class_statements, options));
k = (k + 1);
}
...
let resolved_expr = Object.assign({}, expr);
resolved_expr.left = package_compiler_resolve_identifiers_in_expression(expr.left, manager, function_statements, variable_statements, class_statements, options);
resolved_expr.right = package_compiler_resolve_identifiers_in_expression(expr.right, manager, function_statements, variable_statements, class_statements, options);
return resolved_expr;
}
...
let resolved_expr = Object.assign({}, expr);
resolved_expr.object = package_compiler_resolve_identifiers_in_expression(expr.object, manager, function_statements, variable_statements, class_statements, options);
return resolved_expr;
}
...
resolved_args.push(package_compiler_resolve_identifiers_in_expression(expr.arguments[n], manager, function_statements, variable_statements, class_statements, options));
n = (n + 1);
}
}
resolved_expr.arguments = resolved_args;
...
export function package_compiler_resolve_identifiers_in_statement(stmt, manager, function_statements, variable_statements, class_statements, options) {
if ((stmt == null)) {
return stmt;
}
if ((stmt.type == "LetStatement")) {
let resolved_stmt = Object.assign({}, stmt);
resolved_stmt.value = package_compiler_resolve_identifiers_in_expression(stmt.value, manager, function_statements, variable_statements, class_statements, options);
return resolved_stmt;
}
if ((stmt.type == "ExpressionStatement")) {
let resolved_stmt = Object.assign({}, stmt);
resolved_stmt.expression = package_compiler_resolve_identifiers_in_expression(stmt.expression, manager, function_statements, variable_statements, class_statements, options);
return resolved_stmt;
}
...
let resolved_stmt = Object.assign({}, stmt);
resolved_stmt.condition = package_compiler_resolve_identifiers_in_expression(stmt.condition, manager, function_statements, variable_statements, class_statements, options);
resolved_stmt.thenBranch = package_compiler_resolve_identifiers_in_statement(stmt.thenBranch, manager, function_statements, variable_statements, class_statements, options);
if ((stmt.elseBranch != null)) {
resolved_stmt.elseBranch = package_compiler_resolve_identifiers_in_statement(stmt.elseBranch, manager, function_statements, variable_statements, class_statements, options);
}
return resolved_stmt;
}
...
let resolved_stmt = Object.assign({}, stmt);
resolved_stmt.condition = package_compiler_resolve_identifiers_in_expression(stmt.condition, manager, function_statements, variable_statements, class_statements, options);
resolved_stmt.body = package_compiler_resolve_identifiers_in_statement(stmt.body, manager, function_statements, variable_statements, class_statements, options);
return resolved_stmt;
}
...
resolved_statements.push(package_compiler_resolve_identifiers_in_statement(stmt.statements[i], manager, function_statements, variable_statements, class_statements, options));
i = (i + 1);
}
...
export function package_compiler_generate_single_js_with_options(file_contents, options) {
return package_compiler_compile_asts_with_options(file_contents, "repl", options);
}
export function package_compiler_generate_single_js_standard_with_options(file_contents, options) {
return package_compiler_compile_asts_with_options(file_contents, "standard", options);
}
export function package_compiler_package_compiler_compile_text(source_text) {
let file_contents = {"main.valkyrie": source_text};
return package_compiler_compile_asts(file_contents, "repl");
}
export function package_compiler_package_compiler_compile_text_with_options(source_text, options) {
let file_contents = {"main.valkyrie": source_text};
return package_compiler_compile_asts_with_options(file_contents, "repl", options);
}
...
let sort_result = package_compiler_topological_sort(analyzer, file_contents);
...
let ast = package_parser_parse(tokens);
if ((ast.type != "Program")) {
return {"success": false, "error": ((("Failed to parse " + file_name) + " at ") + JSON.stringify(ast))};
}
let validation_result = package_compiler_validate_namespace_rules(ast, mode);
...
return {"success": false, "error": (("Duplicate main namespace '" + clean_name) + "' found. Each main namespace must have a unique name.")};
} else {
...
package_compiler_add_symbol_to_namespace(manager, current_namespace, stmt.name, "variable", stmt, file_name);
} else if ((stmt.type == "MicroDeclaration")) {
stmt.sourceNamespace = current_namespace;
function_statements.push(stmt);
package_compiler_add_symbol_to_namespace(manager, current_namespace, stmt.name, "function", stmt, file_name);
} else if ((stmt.type == "ClassDeclaration")) {
stmt.sourceNamespace = current_namespace;
class_statements.push(stmt);
package_compiler_add_symbol_to_namespace(manager, current_namespace, stmt.name, "class", stmt, file_name);
} else {
...
return {"success": false, "error": (("Duplicate main namespace '" + current_name) + "' found. Each main namespace must have a unique name.")};
}
...
func_stmt.uniqueName = package_compiler_get_fully_qualified_name(manager, func_stmt.name, func_stmt.sourceNamespace);
m = (m + 1);
}
let n = 0;
while ((n < class_statements.length)) {
let class_stmt = class_statements[n];
class_stmt.uniqueName = package_compiler_get_fully_qualified_name(manager, class_stmt.name, class_stmt.sourceNamespace);
n = (n + 1);
}
let o = 0;
while ((o < variable_statements.length)) {
let var_stmt = variable_statements[o];
var_stmt.uniqueName = package_compiler_get_fully_qualified_name(manager, var_stmt.name, var_stmt.sourceNamespace);
o = (o + 1);
}
...
modified_stmt.body = package_compiler_resolve_identifiers_in_statement(func_stmt.body, manager, function_statements, variable_statements, class_statements, options);
integrated_ast.statements.push(modified_stmt);
} else {
let modified_stmt = Object.assign({}, func_stmt);
modified_stmt.body = package_compiler_resolve_identifiers_in_statement(func_stmt.body, manager, function_statements, variable_statements, class_statements, options);
...
resolved_member.body = package_compiler_resolve_identifiers_in_statement(member.body, manager, function_statements, variable_statements, class_statements, options);
}
...
let resolved_stmt = package_compiler_resolve_identifiers_in_statement(execution_statements[t], manager, function_statements, variable_statements, class_statements, options);
...
package_compiler_add_symbol_to_namespace(manager, manager.current_namespace, stmt.name, "function", stmt, manager.current_file);
} else if ((stmt.type == "ClassDeclaration")) {
package_compiler_add_symbol_to_namespace(manager, manager.current_namespace, stmt.name, "class", stmt, manager.current_file);
} else if ((stmt.type == "LetStatement")) {
package_compiler_add_symbol_to_namespace(manager, manager.current_namespace, stmt.name, "variable", stmt, manager.current_file);
}
i = (i + 1);
}
let j = 0;
...
stmt.uniqueName = package_compiler_get_fully_qualified_name(manager, stmt.name, manager.current_namespace);
} else if ((stmt.type == "ClassDeclaration")) {
stmt.uniqueName = package_compiler_get_fully_qualified_name(manager, stmt.name, manager.current_namespace);
} else if ((stmt.type == "LetStatement")) {
stmt.uniqueName = package_compiler_get_fully_qualified_name(manager, stmt.name, manager.current_namespace);
...
count = (count + package_compiler_count_ast_nodes(node.statements[i]));
i = (i + 1);
}
}
if ((node.body != null)) {
...
export function package_parser_parseExpressionWithPrecedence(parser, minPrecedence, inline) {
let left = package_parser_parseUnaryExpression(parser, inline);
if ((left && (left.type == "ParseError"))) {
return left;
}
while (true) {
let precedence = package_parser_getOperatorPrecedence(parser.current_token.type);
...
let right = package_parser_parseExpressionWithPrecedence(parser, nextMinPrecedence, inline);
...
let args = [];
if ((parser.current_token.type != "RPAREN")) {
let arg = package_parser_parseExpressionWithPrecedence(parser, 0, inline);
...
while ((parser.current_token.type == "COMMA")) {
parser.advance();
arg = package_parser_parseExpressionWithPrecedence(parser, 0, inline);
if ((arg.type == "ParseError")) {
return arg;
}
args.push(arg);
...
let staticCallNode = new package_parser_Node("StaticMethodCall");
...
let staticAccessNode = new package_parser_Node("StaticPropertyAccess");
...
parser.advance();
let index = package_parser_parseExpressionWithPrecedence(parser, 0, inline);
...
if ((parser.current_token.type == "LPAREN")) {
parser.advance();
let expr = package_parser_parseExpressionWithPrecedence(parser, 0, inline);
if ((expr && (expr.type == "ParseError"))) {
return expr;
}
let rparen = parser.expect("RPAREN");
...
let element = package_parser_parseExpressionWithPrecedence(parser, 0, inline);
...
while ((parser.current_token.type == "COMMA")) {
parser.advance();
element = package_parser_parseExpressionWithPrecedence(parser, 0, inline);
if ((element && (element.type == "ParseError"))) {
return element;
}
node.elements.push(element);
...
let value = package_parser_parseExpressionWithPrecedence(parser, 0, inline);
if ((value && (value.type == "ParseError"))) {
return value;
}
...
export function package_parser_parseTypeExpressionWithPrecedence(parser, minPrecedence) {
let left = package_parser_parseUnaryTypeExpression(parser);
if ((left && (left.type == "ParseError"))) {
return left;
}
while (true) {
let precedence = package_parser_getTypeOperatorPrecedence(parser.current_token.type);
if ((precedence < minPrecedence)) {
break;
}
let op = parser.current_token.value;
...
let right = package_parser_parseTypeExpressionWithPrecedence(parser, nextMinPrecedence);
if ((right && (right.type == "ParseError"))) {
return right;
}
if ((tokenType == "ARROW")) {
...
class package_compiler_NamespaceUtils {
  static is_main_namespace(namespace_path) {
    if ((namespace_path == null)) {
      return false;
    }
    return namespace_path.endsWith("!");
  ...
  static get_main_namespace_name(namespace_path) {
    if (package_compiler_NamespaceUtils.is_main_namespace(namespace_path)) {
      return namespace_path.substring(0, (namespace_path.length - 1));
    }
    return namespace_path;
  }

  static add_symbol_to_namespace(manager, namespace_path, symbol_name, symbol_type, symbol_data, file_path) {
    if ((manager.namespaces[namespace_path] == null)) {
      manager.namespaces[namespace_path] = {"symbols": {}, "files": []};
    }
    let namespace_data = manager.namespaces[namespace_path];
    namespace_data.symbols[symbol_name] = {"type": symbol_type, "data": symbol_data};
    if ((namespace_data.files.indexOf(file_path) == -1)) {
      namespace_data.files.push(file_path);
    }
  }

  static add_using_import(manager, using_path, is_global) {
    if ((manager.usings[manager.current_file] == null)) {
      manager.usings[manager.current_file] = [];
    }
    let using_info = {"namespace": using_path, "isGlobal": is_global};
  ...
  static find_symbol_namespace(manager, symbol_name, symbol_type) {
    let namespace_keys = Object.keys(manager.namespaces);
    let i = 0;
    while ((i < namespace_keys.length)) {
      let namespace_name = namespace_keys[i];
  ...
  static get_fully_qualified_name(manager, symbol_name, namespace_path) {
    if (((namespace_path == null) || (namespace_path == ""))) {
      return symbol_name;
    }
    let clean_namespace = package_compiler_NamespaceUtils.get_main_namespace_name(namespace_path);
    return ((clean_namespace.replace("::", "_") + "_") + symbol_name);
  }

  static validate_namespace_rules(ast, mode) {
    let has_namespace = false;
    let has_main_namespace = false;
    let i = 0;
    while ((i < ast.statements.length)) {
  ...
  static find_namespace_provider(namespace_path, file_contents) {
    let file_names = Object.keys(file_contents);
    let i = 0;
    while ((i < file_names.length)) {
      let file_name = file_names[i];
      ...
      if ((tokens.length > 0)) {
        ...
        if ((ast.type == "Program")) {
          ...
          while ((j < ast.statements.length)) {
            let stmt = ast.statements[j];
            if ((stmt.type == "NamespaceStatement")) {
              let declared_namespace = package_compiler_join_path(stmt.path, "::");
              if ((declared_namespace == namespace_path)) {
                return file_name;
              }
            }
            j = (j + 1);
...
class package_compiler_Compiler {
  ...
  compile_asts(file_contents) {
    this.diagnostics = new package_compiler_CompilerDiagnostics();
    return this.compile_asts_with_diagnostics(file_contents);
  }

  compile_asts_with_diagnostics(file_contents) {
    // Reset state
    this.namespace_manager = new package_compiler_NamespaceManager();
    this.dependency_analyzer = new package_compiler_DependencyAnalyzer();
    ...
    while ((i < file_names.length)) {
      let file_name = file_names[i];
      let content = file_contents[file_name];
      this.dependency_analyzer.dependencies[file_name] = [];
      let lexer_instance = new package_lexer_ValkyrieLexer(content);
      let tokens = lexer_instance.tokenize();
      if ((tokens.length > 0)) {
        let ast = package_parser_parse(tokens);
        if ((ast.type == "Program")) {
          ...
          while ((j < ast.statements.length)) {
            ...
            if ((stmt.type == "UsingStatement")) {
              let using_path = package_compiler_join_path(stmt.path, "::");
              let provider_file = package_compiler_NamespaceUtils.find_namespace_provider(using_path, file_contents);
              if (((provider_file != null) && (provider_file != file_name))) {
                this.dependency_analyzer.dependencies[file_name].push(provider_file);
              }
            }
            j = (j + 1);
          }
    ...
    let sort_result = this.dependency_analyzer.topological_sort(file_contents);
    if ((sort_result.error != null)) {
      this.diagnostics.add_error(sort_result.error);
      return {"success": false, "diagnostics": this.diagnostics.get_all_diagnostics()};
    }
    ...
    while ((k < sorted_files.length)) {
      let file_name = sorted_files[k];
      let content = file_contents[file_name];
      this.namespace_manager.current_file = file_name;
      let lexer_instance = new package_lexer_ValkyrieLexer(content);
      let tokens = lexer_instance.tokenize();
      if ((tokens.length == 0)) {
        k = (k + 1);
      ...
      let ast = package_parser_parse(tokens);
      if ((ast.type != "Program")) {
        this.diagnostics.add_error(((("Failed to parse " + file_name) + " at ") + JSON.stringify(ast)), 0, 0, file_name);
        k = (k + 1);
        continue;
      }
      let validation_result = package_compiler_NamespaceUtils.validate_namespace_rules(ast, this.options.mode);
      if (!validation_result.success) {
        this.diagnostics.add_error(validation_result.error, 0, 0, file_name);
        k = (k + 1);
        continue;
      }
        ...
        if ((stmt.type == "NamespaceStatement")) {
          ...
          if (stmt.isMainNamespace) {
            ...
            if ((this.options.mode == "standard")) {
              console.log(("🔍 检查主命名空间: " + current_namespace));
              console.log(("📋 当前管理器中的命名空间: " + Object.keys(this.namespace_manager.namespaces).join(", ")));
              if ((this.namespace_manager.namespaces[current_namespace] != null)) {
                let clean_name = current_namespace.substring(0, (current_namespace.length - 1));
                console.log(("❌ 发现重复的主命名空间: " + clean_name));
                this.diagnostics.add_error((("Duplicate main namespace '" + clean_name) + "' found. Each main namespace must have a unique name."), 0, 0, file_name);
              } else {
                console.log(("✅ 主命名空间检查通过: " + current_namespace));
              }
            }
          }
          this.namespace_manager.current_namespace = current_namespace;
        } else if ((stmt.type == "UsingStatement")) {
          let using_path = package_compiler_join_path(stmt.path, "::");
          let is_global = package_compiler_NamespaceUtils.is_main_namespace(using_path);
          package_compiler_NamespaceUtils.add_using_import(this.namespace_manager, using_path, is_global);
        } else if ((stmt.type == "JSImportStatement")) {
          js_import_statements.push(stmt);
        } else if ((stmt.type == "LetStatement")) {
          stmt.sourceNamespace = current_namespace;
          variable_statements.push(stmt);
          package_compiler_NamespaceUtils.add_symbol_to_namespace(this.namespace_manager, current_namespace, stmt.name, "variable", stmt, file_name);
        } else if ((stmt.type == "MicroDeclaration")) {
          stmt.sourceNamespace = current_namespace;
          function_statements.push(stmt);
          package_compiler_NamespaceUtils.add_symbol_to_namespace(this.namespace_manager, current_namespace, stmt.name, "function", stmt, file_name);
        } else if ((stmt.type == "ClassDeclaration")) {
          stmt.sourceNamespace = current_namespace;
          class_statements.push(stmt);
          package_compiler_NamespaceUtils.add_symbol_to_namespace(this.namespace_manager, current_namespace, stmt.name, "class", stmt, file_name);
        } else {
          execution_statements.push(stmt);
        }
        l = (l + 1);
    ...
    // Continue with duplicate main namespace check, etc.
    if ((this.options.mode == "standard")) {
      let main_namespaces = [];
      let main_namespace_names = [];
      let namespace_keys = Object.keys(this.namespace_manager.namespaces);
      let ns_index = 0;
      while ((ns_index < namespace_keys.length)) {
        let ns_name = namespace_keys[ns_index];
        if (ns_name.endsWith("!")) {
      ...
      while ((duplicate_index < main_namespace_names.length)) {
        ...
        while ((check_index < main_namespace_names.length)) {
          if ((main_namespace_names[check_index] == current_name)) {
            this.diagnostics.add_error((("Duplicate main namespace '" + current_name) + "' found. Each main namespace must have a unique name."));
          }
          check_index = (check_index + 1);
        }
        duplicate_index = (duplicate_index + 1);
    ...
    while ((m < function_statements.length)) {
      let func_stmt = function_statements[m];
      func_stmt.uniqueName = package_compiler_NamespaceUtils.get_fully_qualified_name(this.namespace_manager, func_stmt.name, func_stmt.sourceNamespace);
      m = (m + 1);
    }
    let n = 0;
    while ((n < class_statements.length)) {
      let class_stmt = class_statements[n];
      class_stmt.uniqueName = package_compiler_NamespaceUtils.get_fully_qualified_name(this.namespace_manager, class_stmt.name, class_stmt.sourceNamespace);
      n = (n + 1);
    }
    let o = 0;
    while ((o < variable_statements.length)) {
      let var_stmt = variable_statements[o];
      var_stmt.uniqueName = package_compiler_NamespaceUtils.get_fully_qualified_name(this.namespace_manager, var_stmt.name, var_stmt.sourceNamespace);
      o = (o + 1);
    }
    
    // Generate integrated AST
    let integrated_ast = {"type": "Program", "statements": []};
    let p = 0;
    while ((p < js_import_statements.length)) {
      integrated_ast.statements.push(js_import_statements[p]);
    ...
    while ((r < function_statements.length)) {
      ...
      if (((func_stmt.uniqueName != null) && (func_stmt.uniqueName != func_stmt.name))) {
        let modified_stmt = Object.assign({}, func_stmt);
        modified_stmt.name = func_stmt.uniqueName;
        modified_stmt.body = package_compiler_resolve_identifiers_in_statement(func_stmt.body, this.namespace_manager, function_statements, variable_statements, class_statements, this.options);
        integrated_ast.statements.push(modified_stmt);
      } else {
        let modified_stmt = Object.assign({}, func_stmt);
        modified_stmt.body = package_compiler_resolve_identifiers_in_statement(func_stmt.body, this.namespace_manager, function_statements, variable_statements, class_statements, this.options);
    ...
    while ((s < class_statements.length)) {
      ...
      if ((class_stmt.members != null)) {
        ...
        while ((m < class_stmt.members.length)) {
          ...
          let resolved_member = Object.assign({}, member);
          if ((((member.type == "MemberStatement") || (member.type == "ConstructorStatement")) && (member.body != null))) {
            resolved_member.body = package_compiler_resolve_identifiers_in_statement(member.body, this.namespace_manager, function_statements, variable_statements, class_statements, this.options);
          }
          resolved_members.push(resolved_member);
          m = (m + 1);
        }
    ...
    let t = 0;
    while ((t < execution_statements.length)) {
      let resolved_stmt = package_compiler_resolve_identifiers_in_statement(execution_statements[t], this.namespace_manager, function_statements, variable_statements, class_statements, this.options);
      integrated_ast.statements.push(resolved_stmt);
      t = (t + 1);
    }
    ...
    return {
      "success": !this.diagnostics.has_errors(),
      "code": this.diagnostics.has_errors() ? null : generated,
      "diagnostics": this.diagnostics.get_all_diagnostics()
    };
  }
}
...
this.current_char = this.source.charAt(this.position);
  ...
  skip_whitespace() {
while (((this.current_char != "") && package_lexer_is_whitespace(this.current_char))) {
this.advance();
}
}
  ...
  read_number() {
let result = "";
while (((this.current_char != "") && package_lexer_is_digit(this.current_char))) {
...
return result;
  ...
  read_identifier() {
let result = "";
while (((this.current_char != "") && package_lexer_is_alpha_numeric(this.current_char))) {
result = (result + this.current_char);
this.advance();
}
return result;
...
this.advance();
return new package_lexer_Token("DOUBLE_COLON", "::", line, column);
}
...
return new package_lexer_Token("ERROR", ("Unknown character: " + ch), line, column);
}
